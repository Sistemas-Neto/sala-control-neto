...
import { createBooking, createComboBooking, updateBooking, cancelBookingFromRoom } from "../services/graphService";
...
export default function BookingModal({ rooms, selectedDate, editEvent, editRoom, onClose, onSuccess }) {
  ...
  const handleCancelReserva = async () => {
    ...
    try {
      ...
      setErrorMsg("");
      // Se borra directamente del calendario de la sala (no del calendario personal
      // de quien la creó), para que funcione sin importar quién la haya aprobado.
      const roomEmail = editRoom?.emailAddress || selSala;
      await cancelBookingFromRoom(instance, account, roomEmail, editEvent.id);
      setStatus("cancelled");
